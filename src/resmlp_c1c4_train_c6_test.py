"""ResMLP regressor: train on C1+C4, test on C6.
...
WINDOW_SIZE = 1024
STEP = 512
CORR_THRESHOLD = 0.25  # keep slightly looser filter
N_COMPONENTS = 15      # keep PCA dim fixed as requested
BATCH_SIZE = 128
HIDDEN_SIZE = 256      # wider model
DROPOUT = 0.12         # a bit more retention
EPOCHS = 2000
LR = 3e-5              # slower to reduce oscillation
WEIGHT_DECAY = 1e-5    # light regularization
PATIENCE = 260         # allow more plateau time
SEED = 42
...
class ResMLPRegressor(nn.Module):
    def __init__(self, input_dim: int, hidden_size: int = HIDDEN_SIZE, dropout: float = DROPOUT, num_blocks: int = 4):
        super().__init__()
...
def train_model(model: nn.Module, train_loader: DataLoader, val_loader: DataLoader, y_scaler):
    ...
    for epoch in range(EPOCHS):
        ...
        if mae < best_mae:
            ...
            torch.save(model.state_dict(), "best_resmlp_c1c4_to_c6.pth")
        else:
            no_improve += 1
...
def main():
    ...
    model = ResMLPRegressor(input_dim=input_dim, hidden_size=HIDDEN_SIZE, dropout=DROPOUT).to(DEVICE)
    ...
    plt.xlabel("Epoch")
    plt.title("Training Curve (ResMLP C1+C4 train)")
    plt.legend()
