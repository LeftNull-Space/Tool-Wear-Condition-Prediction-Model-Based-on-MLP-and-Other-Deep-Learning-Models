"""MLP regressor: train on C1+C4, test on C6.
...
def prepare_data():
    ...
    # relative VB baseline on global min of all
...
def train_model(model: nn.Module, train_loader: DataLoader, val_loader: DataLoader, y_scaler):
    ...
    for epoch in range(EPOCHS):
        ...
        if mae < best_mae:
            ...
            torch.save(model.state_dict(), "best_mlp_c1c4_to_c6.pth")
        else:
            no_improve += 1
...
def main():
    ...
    model = MLPRegressor(input_dim=input_dim, hidden_size=HIDDEN_SIZE, dropout=DROPOUT).to(DEVICE)
    ...
    plt.xlabel("Epoch")
    plt.title("Training Curve (C1+C4 train)")
    plt.legend()
